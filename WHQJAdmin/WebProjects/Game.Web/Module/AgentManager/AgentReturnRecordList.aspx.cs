...
namespace Game.Web.Module.AgentManager
{
    public partial class AgentReturnRecordList : AdminPage
    {
        ...
        /// </summary>
        /// <returns></returns>
        protected string GetReturnUser(int sid, int tid)
        {
            string str = string.Empty;
            if (usertable != null && usertable.Rows.Count > 0)
            {
                DataRow[] rows = usertable.Select("UserID=" + sid);
                if (rows != null && rows.Length > 0)
                {
                    str = str + string.Format("<td><a href=\"javascript:;\" class=\"l\" onclick=\"openWindowOwn('/Module/AccountManager/AccountsBaseInfo.aspx?param={0}')\">{1}（{2}）</a></td>",
                ...
                rows = usertable.Select("UserID=" + tid);
                if (rows != null && rows.Length > 0)
                {
                    str = str + string.Format("<td><a href=\"javascript:;\" class=\"l\" onclick=\"openWindowOwn('/Module/AccountManager/AccountsBaseInfo.aspx?param={0}')\">{1}（{2}）</a></td>",
                        rows[0]["UserID"], rows[0]["NickName"], rows[0]["GameID"]);
        ...
        private void BindData()
        {
            ...
            PagerSet pagerSet = FacadeManage.aideAgentFacade.GetPageAgentReturnList(where, "ORDER BY CollectDate DESC", anpNews.CurrentPageIndex, anpNews.PageSize);
