...
namespace Game.Data
{
    ...
    public class GroupDataProvider : BaseDataProvider, IGroupDataProvider
    {
        ...
        {
            string sqlQuery = string.Format(@"SELECT UserID,BattleCount,JoinDateTime FROM IMGroupMember(NOLOCK) WHERE GroupID={0};
            ...
            //    SELECT UserID,GameID,Accounts,NickName FROM WHQJAccountsDB.dbo.AccountsInfo(NOLOCK) WHERE UserID IN(SELECT [MemberID] FROM IMGroupMember(NOLOCK) WHERE GroupID={0})",
            //    groupid);
