...
BEGIN
  ...
  BEGIN
	...
	BEGIN
		SET @strErrorDescribe = N'抱歉，您领取的奖励不能低于' + CAST(@ReceiveDiamondMin AS NVARCHAR(30)) + N'钻石'
		RETURN 1004
	END
  ...
  BEGIN
	...
	BEGIN
		SET @strErrorDescribe = N'抱歉，您领取的奖励不能低于' + CAST(@ReceiveGoldMin AS NVARCHAR(30)) + N'金币'
		RETURN 1004
	END
