...
IF EXISTS (SELECT *
FROM DBO.SYSOBJECTS
WHERE ID = OBJECT_ID(N'[dbo].NET_PW_BindMolible') and OBJECTPROPERTY(ID, N'IsProcedure') = 1)
...
declare @BindMolibleReward int
---绑定手机号奖励
declare @Gameid int----游戏id 
declare @PreScore int
---赠送前金币
declare @PreInsureScore int
---赠送前银行金币
set @RS=-1
select @BindMolibleReward=StatusValue
from SystemStatusInfo
where StatusName='BindMolibleReward'

select @createtime=CollectTime
FROM dbo.CheckCode
where PhoneNum=@Molible and CheckCode=@Code
select @Gameid=isnull(Gameid,0)
from AccountsInfo
where UserID=@Userid

IF EXISTS(SELECT *
FROM dbo.AccountsInfo
WHERE RegisterMobile=@Molible)
BEGIN
...
if @Gameid>0 and @createtime is not null
    begin
  select @PreScore=Score, @PreInsureScore=InsureScore
