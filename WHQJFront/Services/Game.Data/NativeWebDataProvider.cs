...
namespace Game.Data
{
    ...
    public class NativeWebDataProvider : BaseDataProvider, INativeWebDataProvider
    {
        ...
        public DataSet GetAwardHomeList(int typeid)
        {
            ...
            string sqlQuery = @"SELECT TypeID,TypeName,'' as ClassName FROM AwardType(NOLOCK) WHERE Nullity=0 ORDER BY SortID DESC;
                                SELECT AwardID,AwardType,AwardName,AwardImg,UnitPrice FROM AwardInfo(NOLOCK) 
                                WHERE AwardType IN(SELECT TOP 1 TypeID FROM AwardType(NOLOCK) WHERE Nullity=0 " + where + @" 
        ...
        {
            string sqlQuery = @"SELECT AwardID,AwardType,AwardName,AwardImg,UnitPrice FROM AwardInfo(NOLOCK) WHERE AwardID=@AwardID AND Nullity=0;
        ...
        {
            string sqlQuery = @"SELECT OrderID,AwardName,BuyNum,PayTicket,OrderStatus,CollectDate FROM AwardOrder(NOLOCK) 
        ...
        {
            List<DbParameter> prams = new List<DbParameter> { Database.MakeInParam("userid", userid), Database.MakeInParam("strClientIP", ClientIP) };
