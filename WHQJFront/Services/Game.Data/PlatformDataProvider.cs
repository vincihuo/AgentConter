...
namespace Game.Data
{
    ...
    public class PlatformDataProvider : BaseDataProvider, IPlatformDataProvider
    {
        ...
        {
            const string sql =
                "SELECT top 1 * FROM StreamCreateTableFeeInfo WITH(NOLOCK) WHERE RoomID = @RoomID AND GroupID = @GroupID  order by CreateDate desc ";
        ...
        {
            string sql = $"SELECT * FROM GameProperty WITH(NOLOCK) WHERE ID={id} and ExchangeType={ExchangeType} ";
        ...
        public DataSet GetHallBattleRecord(int userid, int typeid)
        {
            ...
            return Database.ExecuteDataset(CommandType.StoredProcedure, "NET_PW_GetHallBattleRecord", prams.ToArray());
        }
        /// <summary>
        /// 获取俱乐部战绩
        /// </summary>
        ...
        public DataSet GetClubBattleRecord(int userid, int groupid, DateTime start, DateTime end)
        {
            List<DbParameter> prams = new List<DbParameter>
            {
                Database.MakeInParam("dwGroupID", groupid),
                Database.MakeInParam("dwUserID", userid),
                Database.MakeInParam("dwStartTime", start),
                Database.MakeInParam("dwEndTime", end)
            };
            return Database.ExecuteDataset(CommandType.StoredProcedure, "NET_PW_GetClubBattleRecord", prams.ToArray());
        }
        #endregion
